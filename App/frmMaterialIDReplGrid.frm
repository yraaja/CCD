...
Begin VB.Form frmMaterialIDReplGrid 
   Caption         =   "ID Replacement Grid"
   ClientHeight    =   6855
   ClientLeft      =   2265
   ClientTop       =   2835
   ClientWidth     =   11130
   Icon            =   "frmMaterialIDReplGrid.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   6855
   ScaleWidth      =   11130
   ...
   End
End
Attribute VB_Name = "frmMaterialIDReplGrid"
...
'    strSelect = "select Material.*, Material.last_update_id as 'mat_last_update_id' from Material where "
'    strSelect = strSelect + "mat_id = '0'"
...
Private Sub cmdSearch_Click()
    ...
    If Len(MaterialID.Text) = 0 And Len(Description.Text) = 0 Then
    ...
    strSelect = "select mat_skey, mat_id, alt_mat_id, tech_desc, metric_tech_desc, active_status_ind, purchase_unit, purchase_usage_conv_factor, usage_unit, " + _
        "last_update_id, last_update_date, last_update_person from Material where " '
'    strSelect = "select mat_skey, mat_id, alt_mat_id, tech_desc, metric_tech_desc, active_status_ind, purchase_unit, purchase_usage_conv_factor, usage_unit, " + _
'        "last_update_id as 'mat_last_update_id', last_update_date as 'mat_last_update_date', last_update_person as 'mat_last_update_person' from Material where " ' + _
'         "start_date <= '" + Format(dtmToday, "mm/dd/yyyy") + "' and term_date >= '" + Format(dtmToday, "mm/dd/yyyy") + "'"
    
    If Not MaterialID.Text = "" Then
        strSelect = strSelect + " mat_id like '"
        If Not Left(MaterialID.Text, 1) = "M" Then
            strSelect = strSelect + "M"
        End If
        strSelect = strSelect + SQLChangeWildcard(MaterialID.Text) + "'"
    End If
    If Not Description.Text = "" Then
        If Not MaterialID.Text = "" Then
            strSelect = strSelect + " and"
        End If
        strSelect = strSelect + " tech_desc LIKE '" + SQLFixString(SQLChangeWildcard(Description.Text)) + "'"
    End If
    strSelect = strSelect + " ORDER BY mat_id"
